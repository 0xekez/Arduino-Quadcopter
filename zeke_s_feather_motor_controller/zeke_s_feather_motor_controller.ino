...
// lists of two floats
// only two beacuse we're concerned with two axis
...
void setup() {
  ...
  // 9600 chosen because we only have 8hz processor
  Serial.begin(9600);
...
void loop() {
  ...
  *part that afects the angles and ofcourse multiply by 0.98 */
  ...
  // max that we can add or subtract is 250
  // TODO: mess with this
  if(PID < -250)
    {
      PID= -250;
  ...
  // make sure again that we don't pass the max vals for throttle
  if(pwmRight < 0)
  {
    pwmRight= 0;
  }
  ...
  drive_motor(GATE_L, pwmLeft);
  drive_motor(GATE_R, pwmRight);
